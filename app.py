...
def predict():
    ...
    input_data = np.array([[T_Max, T_Min, Humidity, Speed, WindPres]])
    scaler=MinMaxScaler()
    ...
    output5 = prediction[0][4]
    out = model1.predict([[output1, output2, output3, output4, output5]])
    if out == 3:
    ...
    return render_template('result.html', output1=output1, output2=output2, output3=output3, output4=output4, output5=output5, output6=output6)  # Render result.html page with output values
